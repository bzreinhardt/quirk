...
% curY = sum(intY);
% curZ = sum(intZ);
% curX2 = -1*integral(intX2,0,2*pi-0.1);
% curY2 = -1*integral(intY2,0,2*pi-0.1);
% curZ2 = -1*integral(intZ2,0,2*pi-0.1);
...
% vxB_y_x = vxB(x_x,y_x,z_x,mMag,xMag,omMag_bod,vMag_bod,'y');
...
xlabel('y');ylabel('z');
subplot(212)
% quiver(y_x,z_x,vxB_y_x,vxB_z_x);
% xlabel('y');ylabel('z');
...
xlabel('z');ylabel('x');
subplot(212)
% quiver(z_y,x_y,vxB_z_y,vxB_x_y);
% xlabel('z');ylabel('x');
